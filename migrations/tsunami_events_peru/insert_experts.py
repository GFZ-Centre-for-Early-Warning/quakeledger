...
def main():
    ...
    new_events_db_file = os.path.join(current_dir, "peru_events_sqlite3.db")
    old_events_db_file = os.path.join(current_dir, "..", "..", "sqlite3.db")
    ...
    conversions = {"eventID": str, "Identifier": str, "rake": float}
    for column_to_convert in conversions.keys():
        conv_fun = conversions[column_to_convert]
        new_events[column_to_convert] = new_events[column_to_convert].apply(
            conv_fun
        )
    ...
    new_events.to_sql(
        "events", old_events_db_engine, if_exists="append", index=False
    )
