...
def events2quakeml(catalog, provider="GFZ"):
    ...
    quakeml = le.Element(
        "eventParameters", xmlns=xml_namespace, publicID=add_id_prefix("0")
    )
    # go through all events
    for i in range(len(catalog)):
        quake = catalog.iloc[i]
        event = le.SubElement(
            quakeml, "event", {"publicID": add_id_prefix(str(quake.eventID))}
        )
        ...
        origin = le.SubElement(
            event, "origin", {"publicID": add_id_prefix(str(quake.eventID))}
        )
        origin = add_uncertain_child(
            origin,
            childname="time",
            value=event2utc(quake),
            uncertainty=format_xsdouble(quake.timeUncertainty),
        )
        ...
        origin = add_uncertain_child(
            ...
            uncertainty=format_xsdouble(quake.latitudeUncertainty),
        )
        ...
        origin = add_uncertain_child(
            ...
            uncertainty=format_xsdouble(quake.longitudeUncertainty),
        )
        ...
        origin = add_uncertain_child(
            ...
            uncertainty=format_xsdouble(quake.depthUncertainty),
        )
        ...
        azimuthMaxHorizontalUncertainty.text = format_xsdouble(
            quake.azimuthMaxHorizontalUncertainty
        )
        # magnitude
        magnitude = le.SubElement(
            event, "magnitude", {"publicID": add_id_prefix(str(quake.eventID))}
        )
        magnitude = add_uncertain_child(
            magnitude,
            childname="mag",
            value=str(quake.magnitude),
            uncertainty=format_xsdouble(quake.magnitudeUncertainty),
        )
        ...
        focalMechanism = le.SubElement(
            event,
            "focalMechanism",
            {"publicID": add_id_prefix(str(quake.eventID))},
        )
        nodalPlanes = le.SubElement(
            focalMechanism, "nodalPlanes", {"preferredPlane": "1"}
        )
        nodalPlane1 = le.SubElement(nodalPlanes, "nodalPlane1")
        nodalPlane1 = add_uncertain_child(
            nodalPlane1,
            childname="strike",
            value=str(quake.strike),
            uncertainty=format_xsdouble(quake.strikeUncertainty),
        )
        ...
        nodalPlane1 = add_uncertain_child(
            ...
            childname="dip",
            value=str(quake.dip),
            uncertainty=format_xsdouble(quake.dipUncertainty),
        )
        ...
        nodalPlane1 = add_uncertain_child(
            ...
            uncertainty=format_xsdouble(quake.rakeUncertainty),
        )
...
def get_uncertain_child(parent, childname):
    ...
    try:
        uncertainty = float(
            parent.find(childname).findtext(add_namespace("uncertainty"))
        )
    except:
...
def quakeml2events(quakemlfile, provider="GFZ"):
    ...
    for i, event in enumerate(quakeml):
        ...
        catalog.iloc[i].type = event.find(
            add_namespace("description")
        ).findtext(add_namespace("text"))
        # origin
        ...
        catalog.iloc[i].timeUncertainty = float(
            origin.find(add_namespace("time")).findtext(
                add_namespace("uncertainty")
            )
        )
        # latitude/longitude/depth
        (
            catalog.iloc[i].latitude,
            catalog.iloc[i].latitudeUncertainty,
        ) = get_uncertain_child(origin, add_namespace("latitude"))
        (
            catalog.iloc[i].longitude,
            catalog.iloc[i].longitudeUncertainty,
        ) = get_uncertain_child(origin, add_namespace("longitude"))
        (
            catalog.iloc[i].depth,
            catalog.iloc[i].depthUncertainty,
        ) = get_uncertain_child(origin, add_namespace("depth"))
        # agency/provider
        catalog.iloc[i].agency = origin.find(
            add_namespace("creationInfo")
        ).findtext(add_namespace("value"))
        # magnitude
        magnitude = event.find(add_namespace("magnitude"))
        (
            catalog.iloc[i].magnitude,
            catalog.iloc[i].magnitudeUncertainty,
        ) = get_uncertain_child(magnitude, add_namespace("mag"))
        # originUncertainty
        originUncertainty = origin.find(add_namespace("originUncertainty"))
        catalog.iloc[i].horizontalUncertainty = originUncertainty.find(
            add_namespace("horizontalUncertainty")
        ).findtext(add_namespace("value"))
        catalog.iloc[i].minHorizontalUncertainty = originUncertainty.find(
            add_namespace("minHorizontalUncertainty")
        ).findtext(add_namespace("value"))
        catalog.iloc[i].maxHorizontalUncertainty = originUncertainty.find(
        ...
        # plane
        nodalPlanes = event.find(add_namespace("focalMechanism")).find(
            add_namespace("nodalPlanes")
        )
        preferredPlane = nodalPlanes.get("preferredPlane")
        preferredPlane = nodalPlanes.find(
            add_namespace("nodalPlane" + preferredPlane)
        )
        # GET uncertain child!!
        (
            catalog.iloc[i].strike,
            catalog.iloc[i].strikeUncertainty,
        ) = get_uncertain_child(preferredPlane, add_namespace("strike"))
        (
            catalog.iloc[i].dip,
            catalog.iloc[i].dipUncertainty,
        ) = get_uncertain_child(preferredPlane, add_namespace("dip"))
        (
            catalog.iloc[i].rake,
            catalog.iloc[i].rakeUncertainty,
        ) = get_uncertain_child(preferredPlane, add_namespace("rake"))
